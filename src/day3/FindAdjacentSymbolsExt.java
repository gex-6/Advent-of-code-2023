...
public class FindAdjacentSymbolsExt {
        ...
        for (int row = 0; row < gameArray.length; row++) {
            ...
            for (int col = 0; col < currentCol.length(); col++) {
                ...
                if (ch == '.') {
                    ...
                    int gearRatios = calculateGearRatios(gameArray, logicArr, row, col);
    ...
    private static int calculateGearRatios(String[] gameArray, boolean[][] logicArr, int row, int col) {
        int gearRatios = 0;
        int number1 = 0, number2 = 0;

        for (int x = -1; x < 2; x++) {
            for (int y = -1; y < 2; y++) {
                if (checkBorder(logicArr, col + x, row + y) && !logicArr[row + y][col + x]) {
                    char currentCh = gameArray[row + y].charAt(col + x);
                    if (Character.isDigit(currentCh)) {
                        String newNumber = checkNeighbours("" + currentCh, gameArray, logicArr, row + y, col + x, -1);
                        newNumber = checkNeighbours(newNumber, gameArray, logicArr, row + y, col + x, 1);

                        if (number1 == 0) {
                            number1 = Integer.parseInt(newNumber);
                        } else {
                            number2 = Integer.parseInt(newNumber);
                        }
                    }
                }
            }
    ...
    private static String checkNeighbours(String newNumber, String[] gameArray, boolean[][] logicArr, int y, int x, int dir) {
        int newX = x + dir;

        if (checkBorder(logicArr, newX, y) && !logicArr[y][newX] && Character.isDigit(gameArray[y].charAt(newX))) {
            logicArr[y][newX] = true;
            newNumber = (dir > 0) ? newNumber + gameArray[y].charAt(newX) : gameArray[y].charAt(newX) + newNumber;
            newNumber = checkNeighbours(newNumber, gameArray, logicArr, y, newX, dir);
        }

        return newNumber;
    }

    private static boolean checkBorder(boolean[][] logicArr, int x, int y) {
